...
def complex_to_real_transformation(sizes):
    ...
    for i in range(len(sizes)):
        ...
        for j in range(int(lval)):
            transformation_matrix[j][j] = 1.0j
            transformation_matrix[j][sizes[i]-j-1] = st*1.0j
            transformation_matrix[int(sizes[i]-j-1)][j] = 1.0
            transformation_matrix[int(sizes[i]-j-1)][int(sizes[i]-j-1)] = st*-1.0
            st = st * -1.0
        transformation_matrix[int(lval)][int(lval)] = np.sqrt(2.0)
